...
### Added
- **`vanishpp:vanished` LuckPerms context now works on the Velocity proxy, not just Paper:** Paper has had this context (usable in LuckPerms permission conditions) since earlier work, but the proxy side had no equivalent — a permission conditioned on it would only ever evaluate correctly at the backend, never at the proxy. Added a mirrored `LuckPermsHook` on `vanishpp-velocity`, sourced from the proxy's own cross-server vanish state (`ProxyStateManager`, already kept in sync via the existing plugin-messaging protocol) rather than a Paper plugin instance, which doesn't exist on that side. Registration is optional (only if LuckPerms is present on the proxy) and mirrors Paper's exact context key so one permission condition behaves consistently whether it's checked at the proxy or a backend server. Verified end-to-end on a real Velocity proxy + Paper backend + LuckPerms-Velocity: vanishing/unvanishing on the backend flips the proxy-side context value live (`false → true → false`), no reconnect needed.

### Fixed
- **`/vspec` action bar flickered between "you are vanished" and the spectate overlay:** `/vspec` shares its target-tracking with `/vfollow`, whose periodic task drives the spectate-info action bar — but that raced, uncoordinated, against a completely separate general "you are vanished" heartbeat that had no idea spectate/follow mode existed, so the two kept overwriting each other's action bar text. The generic heartbeat now skips any player currently in an active `/vspec`/`/vfollow` session.
- **`/vspec stop` could leave the player stuck in SPECTATOR gamemode:** unlike `/vfollow`, `/vspec` saved the player's gamemode *unconditionally* on every invocation — so switching `/vspec` targets twice, using `/vfollow` then `/vspec`, or double-shift-toggling into spectator before `/vspec`, would each re-save the already-current `SPECTATOR` gamemode over the real original, permanently losing it. The codebase actually had **four independent, uncoordinated places** tracking "gamemode to restore" (the shared `/vspec`+`/vfollow` map, plus two separate player-metadata keys for the double-shift toggle and the vanish/unvanish toggle) — consolidated into a single guarded store (`Vanishpp.saveGamemodeForRestore()`/`restoreSavedGamemode()`, save-if-absent semantics) used by all four. As part of this, unvanishing while still mid-`/vspec`/`/vfollow` now also cleanly drops the active follow/spectate session instead of leaving stale tracking state behind (which previously could cause the follower to keep getting silently re-teleported to the target after they'd already unvanished). Verified on a real server: `/vfollow` → `/vspec` → `/vspec stop`, two sequential `/vspec` calls, and unvanish-mid-spectate on a non-bypass account all correctly restore the real original gamemode.
- **Vanished player's nametag could leak through a third-party team plugin (e.g. BetterTeams) to a non-seer, as floating text with no player model:** a prior fix attempt (already shipped) scrubbed `SCOREBOARD_TEAM` packets but failed *open* on any exception — if a packet didn't match the expected shape (plausible for a different plugin's team packets), the original, unscrubbed packet was forwarded rather than dropped, which is the worst failure direction for a leak-class bug. That scrub (both the plugin's own team and the generic any-other-team path) now fails *closed*: cancel the packet rather than forward it if it can't be verified safe. Separately, `PLAYER_INFO`/`PLAYER_INFO_UPDATE` packets were never intercepted at all — `hidePlayer()` only sends a one-shot removal at the moment visibility changes, so a later tab-list re-sync from another plugin could still re-include a vanished player for a non-seer. Added a backstop listener (`VanishPlayerInfoPolicy`, unit-tested the same way as the existing `VanishTeamPacketPolicy`) that strips vanished-player entries from every such packet regardless of what caused it. **Known residual risk, not closed by this fix:** a client that already cached a third-party team/tab association for a player *before* they vanished won't have it retroactively retracted — both fixes only filter packets going forward from the moment of vanish. Closing that fully would require Vanish++ to proactively manage third-party team membership on vanish, which is separate, larger scope. Verified on a real server with raw packet capture: a vanished player joining a fresh vanilla team (standing in for a third-party team plugin, same underlying API) never leaks their name to a non-seer observer.
...
- **`/vconfig gui` settings mostly did nothing:** 20 of the 23 settings exposed in the Config GUI wrote to config keys that either don't exist or were the wrong path — clicking played the success sound and showed "saved," but nothing ever read the value back, so it had zero effect on real plugin behavior (this is almost certainly what looked like "clicking does nothing" — reported separately). Phantom settings with no real backing config value were removed; wrong-path settings with a real underlying feature were corrected to their actual path (`vanish-appearance.action-bar.enabled`, `vanish-gamemodes.enabled`/`default-spectator`, `flight-control.vanish-enable-fly`, `invisibility-features.prevent-sleeping`, `permissions.default-vanish-level`/`default-see-level`/`max-level`). Settings whose real path is gated behind `/vconfig`'s `SENSITIVE_PREFIXES` + `--confirm` safety requirement (`storage.type`, `storage.redis.*`, `permissions.layered-permissions-enabled`) were removed from the GUI entirely rather than given a bypass — the GUI has no confirmation flow, so exposing them there would let a single accidental click do what the text command deliberately requires an extra confirmation step for. Edit those via the text command instead.
- **`/vconfig gui` pagination math was off by 4 per page (latent):** `ITEMS_PER_PAGE` was computed as `(rows*9)-2` = 25, but the actual per-page rendering capacity is `rows*7` = 21 (every wrapped row reserves its own indent, not just one indent total). No category has more than 21 settings today so this never surfaced, but a future 22-25-setting category would have silently dropped items off the end of the page with no error. Fixed the constant to match the real capacity.
- **Admin Dashboard's Close button did nothing:** The barrier item's click was cancelled (as intended, to block normal inventory interaction) but never actually closed the inventory — same "I clicked it and nothing happened" failure mode as the Config GUI bug above, just in the other admin GUI.
- **`/vhistory` called `Bukkit.getPlayer()` off the main thread:** `resolveUUID()` ran inside the command's async lookup task — unsafe on Bukkit/Paper (main-thread-only API) and would break outright on Folia's region-threading model. The online-player lookup now happens synchronously before the async task is dispatched; only the offline-player cache lookup (a plain thread-safe map read) happens inside the async block.
- **`/vfollow` didn't actually follow a moving target:** The camera-lock/re-teleport logic only ran off the *follower's own* movement — if the follower stood still while the target walked away, nothing re-synced them at all. Added a periodic Folia-safe sweep (dispatched per-follower through the entity's own region scheduler) that keeps the follower's camera glued to the target's current position regardless of whether the follower moves.
- **Verified, not changed: the silent-container mechanic doesn't have a duplication-risk bug.** Re-verified (after a specific concern was raised) that opening a container silently always references the real, live `Inventory` object directly — never a snapshot/copy that could be written back over concurrent changes from a real player on close. Confirmed with a real two-player concurrent-access test (an admin silently viewing a chest while a survival player removed/added items to the same chest) — the final state exactly matched the survival player's changes, item count conserved exactly, admin's close had zero effect. This class of bug *did* exist in a much older version of this codebase (pre-1.1.7) and was already fixed before this session; this just re-confirms it stayed fixed. Also broadened `vanishpp.silentchest`'s `plugin.yml` description, which still said "chests" only despite covering all container types (barrels, shulker boxes, ender chests too).
